...
// GetToken returns a jwt token generated by the secret.
func (s *SecretController) GetToken(c *gin.Context) {
	username := c.GetString(middleware.UsernameKey)
	name := c.Param("name")
	s.log.X(c).Infow("secret get-token params", "name", name)

	token, err := s.srv.Secrets().GetToken(c, username, name)

	httputil.WriteResponse(c, err, token)
}
